...
class ToolEnvWrapper(EnvWrapper):
    def __init__(self, env: Env, tools: List[BaseTool], tool_use_reward: float = 0.1, max_tool_uses: Optional[int] = None):
        super().__init__(env)
        self.tools = tools
        self.tool_use_reward = tool_use_reward
        self.max_tool_uses = max_tool_uses if max_tool_uses is not None else float('inf')
    ...
    def step(
        ...
        for tool in self.tools:
            ...
            if valid:
                if verbose: print(f"Action {action!r} executed by tool {tool.tool_type}")
                ...
                if verbose: print(f"Action {action!r} not executed by tool {tool.tool_type}")
        ...
        if valid and self.tool_use_counter >= self.max_tool_uses:
            if verbose: print(f"Tried to use tool {tool.tool_type} but max tool uses {self.tool_use_counter} has been reached.")
            observation = f"Tool {tool.tool_type} has reached its maximum usage limit of {self.max_tool_uses}. " + observation
